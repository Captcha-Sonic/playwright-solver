/**
 * Geetest v4 — Nine-Grid Solver (Playwright)
 * ===========================================
 * CaptchaSonic classifies the 9 Geetest tiles and returns which to click.
 * Playwright locates and clicks the correct tiles.
 *
 * Usage:
...
async function getGeetestTiles(page: Page): Promise<{ tiles: Buffer[]; prompt: string }> {
  ...
  const prompt = await page.$eval(
    '.geetest_tips_title',
    (el) => el.textContent?.trim() ?? 'Select matching items'
  ).catch(() => 'Select matching items');
